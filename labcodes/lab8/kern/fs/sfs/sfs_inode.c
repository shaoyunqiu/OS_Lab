...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2012011367 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    ...
    if(nblks > 0){
        ...
        if((ret = sfs_block_op(sfs, buf, ino, nblks))){
            goto out ;
        }
        alen += (size*nblks) ;
        buf += (size*nblks) ;
